...
### 사용자 요청
- Tier 2 ② — 오늘 발견한 "같은 그림이 절마다 설명 있기/없기 들쭉날쭉" 해소.

### 추론 / 결정
- 원인(오늘 실측): 소민섭 방식은 절 생성 시 개념별로 `explain_figure`(related=true/false 판정) → 절마다 개념이 달라 판정이 갈리고, **성공한 설명을 저장 안 해** 재사용 불가. 실측 채움률 4/10.
- 처방 = **캐싱 하이브리드**(옛 ingest "1회 저장" 장점 + 소민섭 "생성 시점·근거 좁힘" 유지): 성공한 설명을 `doc_figures.description`(mig 0023, 지금껏 미사용)에 캐싱 → 이후 절·재생성에서 재사용.
- 저장 위치: `_attach_section_figures`가 [저장] **직렬 루프** 안에서 돌므로(병렬 gather는 순수 LLM만) gather 밖 순차 write 안전. `f.description = expl` 직접 세팅(커밋은 저장 루프 끝 db.commit).
- 트레이드오프: 첫 성공 절의 개념 프레이밍이 굳지만 그림 자체 안내 설명이라 무해, '절마다 빈 그림'보다 나음.

### 한 일
- `service._attach_section_figures._explain`: `if f.description: return`(캐시 재사용, LLM 콜 0) 선행. gather 후 새 설명만 `f.description`에 캐싱.

### 검증
- 데모 코스 ch1 재생성 2회: 채움 **4/10 → 6/10(재생성1, 캐시 2/3) → 10/10(재생성2, 완전 일관)**. 근거 없는 그림(이미지출처 URL만)은 **캐시 안 됨(2/3)** = related=false 유지, 지어내지 않는 안전장치 보존 ✅.
- 한계(정직): 브랜드 새 챕터 첫 생성은 앞 절이 캐시 전 실행돼 6/10 → 재생성/재열람 시 캐시 warm되며 10/10 수렴. 데모 전 챕터 워밍 권장.

### 다음 액션
1. Tier 2 ③ 문항 정답유출(concept_name=정답 차단)
2. work push(현재 ahead 2)
...
### 사용자 요청 / 맥락
- 멘토링 신청서용 데모 강화: grounding이 정적 배지("근거 있음")에 머물러 "이게 그 근거다"까지 못 보여줌 → **배지 클릭 시 실제 교재 원문**을 띄워 GPT와의 차별(환각 없음·추적 가능)을 화면으로 증명.

### 한 일
- 백엔드(additive): `GET /chunks/evidence?ids=` — 봉투의 `sourceChunkIds`로 doc_chunks 원문(content·page·heading) 조회(`repo.get_chunks_by_ids`, 입력 순서 유지, 잘못된 UUID 무시). `ChunkEvidence` 스키마.
- 프론트: `chunkEvidence.ts` API + `EvidenceBadge` 교재 배지를 **버튼화**("교재 4쪽 · 원문 보기") → 클릭 시 팝업(원문 청크 카드, 페이지·heading 라벨, 이미지 placeholder 마크다운 스트립, "지어낸 내용 아님" 문구). 다크 스코프(.focus-root) 안에선 팝업도 자동 다크 → 몰입 모드 호환.

### 검증
- 엔드포인트: 청크 2개 원문·페이지 정상 반환. tsc 0. (팝업 UI 픽셀은 사용자 육안.)
- 재료 재사용: 봉투 sourceChunkIds + doc_chunks(기존)로 마이그레이션 0·계약 additive.

### 데모 활용
- 1막(커리큘럼 생성 직후) 또는 학습 화면에서 "교재 N쪽 · 원문 보기" 클릭 → 원문 팝업 = "AI가 근거를 대고, 그 원문을 즉석에서 보여준다" 장면.

### 후속 보정 (사용자 피드백)
- **원문 보기를 페이지 배지와 분리**: 페이지 라벨은 좁을 때만(≤5p) 붙이되, **book 블록에 근거 청크가 있으면 "교재 원문 보기"는 항상** 노출(`canShowSource`). 청크가 큰 교재(데이터통신)에서 배지가 억제돼 원문 추적이 안 되던 것 해소 → 어느 절이든 한글 원문 근거 확인 가능.
- **영문 원문 이슈**: 팝업은 교재 원문 그대로라 영문 교재(1장_HCI 영문판)면 영문. 데모는 한글 교재(데이터통신)로 — 검증: 데이터통신 book 블록 원문 보기 = 한글("01. 데이터 통신의 개념과 역사…") ✅.
- **테스트 코스 정리**: 다이어그램검증_OSI·Layer2검증_신호그림·C9병렬검증 삭제 + 1장_HCI 영문판 삭제(영문 원문 원인). 남음: 데이터 통신 + 1장_HCI 한글판.

### 후속 2 — 근거 범위 좁힘 (청크 전체 → 블록 관련 문단)
- **피드백**: 원문 팝업이 청크 전체(섹션 단위, 217문단)라 "이 책 어딘가"처럼 넓음.
- **해결**: `GET /chunks/evidence?blockId=`에 blockId를 받아 **블록 텍스트(`_block_claim_text`) 키워드와 겹치는 문단만** 추림 — 문단 분할(이미지·헤딩 기호 제거) → 키워드 겹침 ≥2 후보 → 점수 상위 12 → 원문 순서 복원 + 중복 제거. 겹침 후보 없으면 전체 폴백. 반환을 `chunks`(청크 전체) → `passages`(문단+page)로 변경.
- **검증**: 데이터통신 concept 블록 217문단 → **11문단**, 관련 원문만 정확(데이터 통신 정의·통신·교환 과정·역사). 마이그 0·계약 additive(blockId 옵션). tsc 0.
- 정밀 귀속의 정공은 여전히 faithfulness 부산물(②) — 지금은 키워드 휴리스틱으로 즉효.

### 후속 3 — "언급된 페이지 전체 보기"(맥락 확장)
- **피드백**: 좁힌 문단만 딸랑 보이면 청크 단위 맥락이 손실됨 → 클릭하면 그 근거가 언급하는 **페이지 내용 전체**를 볼 수 있게(청크 전체 말고 페이지 경계).
- **해결**: 페이지 경계는 `doc_chunks.content`(마크다운 통짜)엔 없고 `refined_elements`(파싱 시 요소별 page)에 있음 → `repo.get_page_content(doc, from, to)`가 요소를 page로 정확히 잘라 반환(머리말·꼬리말 등 제외). `GET /documents/page-content`. chunkEvidence 응답에 documentId·pageFrom·pageTo(좁힌 문단들의 페이지 범위) 추가.
- **프론트**: 팝업에 좁힌 문단(기본) + "📖 언급된 교재 N쪽 전체 보기" 버튼 → 클릭 시 해당 페이지 요소 전체 확장 표시.
- **검증**: 페이지 범위가 청크 전체(3~28)가 아니라 좁힌 문단 기준(3~12)으로, page-content가 페이지별 요소 반환. tsc 0, 마이그 0.

---

## 2026-07-21 (세션35) — Claude CLI — 몰입 뷰어 폴리시(미니맵 클릭 토글·배지 잘림) + integration 머지

### 사용자 요청 (육안 피드백)
- 우상단 미니맵이 이상하게 보임(축소 160×200에 37절이 안 들어가 잘림) → 클릭해야 보이게.
- 블록 카드의 "교재 출처"·비유 배지가 상단에서 잘림.

### 한 일 (frontend)
- 배지 잘림: `fv-card`가 `overflow-y:auto`라 BlockShell 배지(카드 위 −10px)가 클립 → `padding-top:18px`로 배지를 카드 안에 넣음.
- 미니맵: 축소 미니맵 제거 → **우상단 토글 버튼(🗺+현재 위치)** + 클릭 시 중앙 확장 오버레이(`flex-wrap`으로 절 많아도 줄바꿈 조망) + 뒷배경 클릭 닫기. 호버 로직·hitbox 제거.
- tsc 0, HMR 반영.

### integration 머지
- 신청서(멘토링) 제출용으로 `feat/yoonhs-work` → `feat/yoonhs-integration` FF 머지(몰입 뷰어·근거 배지·이 폴리시 포함). 신청서가 integration을 최신으로 가리키게.

---

## 2026-07-20 (세션34) — Claude CLI — 몰입 학습 모드: 목업 2D 매트릭스 뷰어 React 포팅

### 사용자 요청
- 기존 "몰입 뷰어" 토글이 좌패널만 숨겨 사실상 무동작 — 목업(`UXUI_ANT/focus.html`)의 검은 화면 몰입 모드와 동일하게 구현.

### 추론 / 결정
- 목업 = 다크(#111) 전체화면 + 2D 매트릭스(X=가로 슬라이드, Y=블록 자석 스냅) + 줌아웃→팬→줌인 카메라 연출 + 2D 미니맵. React 포팅 안 된 채 이름만 남아 있던 것.
- **다크 전환**: Tailwind v4 `@theme` 토큰을 `.focus-root` 스코프에서 재선언 → 기존 블록 컴포넌트 무수정 다크화(+`.bg-white` 등 리터럴 클래스 최소 오버라이드).
- **실데이터 각색**(목업은 전 블록 정적 가정): X축=절, 활성 절만 실블록 렌더, 이웃 절은 상태 카드(잠김/완료/이동 안내), 잠긴 절 이동 불가(서버 잠금 미러), 절 마지막 셀에 진행 게이트(인출 완료/다 읽었어요) — 잠금 규칙이 몰입 중에도 유지. 채점·선행 삽입 신호는 일반 모드와 동일 라운드트립 재사용(컴팩트 배너).

### 한 일
- 신규 `pages/learning/FocusViewer.tsx` + `focus.css`(목업 CSS 포팅). `LearningPage.tsx` 배선: 몰입 시 일반 레이아웃 대체, 튜터 FAB 숨김(목업 focus-mode 규칙). 키보드 ↑↓←→/ESC, 미니맵 호버 확장+점 클릭 점프, 위치 표시.
- 버그 수리(구현 중 발견): 프로그램 스크롤 ↔ 수동 스크롤 동기화 경합으로 y가 되돌아가는 문제 — 디바운스(120ms)+programmatic 가드(800ms).

### 검증
- placement worktree에서 로컬 vite(45173, mlv2 백엔드 48001 프록시)로 라이브 구동: 다크 루트·37절 컬럼·블록 카드 다크(#1e1e1e) ✓ / ↓ 블록 이동 2/9 ✓ / ← 절 이동 시 줌아웃→translateX→줌인 시퀀스(200/600/1100ms 스냅샷) ✓ / 이동한 절 블록 자동 로드 ✓ / ESC 복귀(헤더·패널·FAB 원상) ✓ / tsc 0.
- 미실행: 몰입 모드에서 실답안 제출(실DB 오염 방지 — 경로는 일반 모드와 동일 onAnswer).

### 열린 이슈
- [ ] 몰입 모드 폴리시: mermaid diagram 다크 대비 확인 / 모바일(터치) 미검토 / 미니맵이 절 단위 점 1개(비활성 절) — 목업처럼 전 블록 점을 원하면 절 블록 수 사전 로드 필요

### 다음 액션
1. 사용자 육안 검증(55173, work 마운트 스택) → 이상 없으면 integration FF 머지
...
### 참고
- "교재 3–28쪽"은 청크가 커서 넓음 — 근거 신뢰 어필엔 ai_prereq 링크가 주력.

### 후속 보정(같은 세션) — 귀속 정밀도: 절 전체 → 대표 청크 + 정확할 때만 주장
- **피드백**: sourcePages가 절 생성 근거 청크 **전 합집합**(=문서 거의 전체 1–28쪽)이라 배지가 "이 책 어딘가"가 돼 신뢰를 깎음. 블록별 귀속이 아니라 절 단위 일괄 스탬프였던 것.
- **처방 ①(적용)**: `get_concept_anchor_pages` — 블록 개념의 **대표 청크(concepts.source_chunk_id, 1청크)** 페이지로 좁힘(serializer `concept_pages` 주입, 기존 `_pages_from_chunks`/`get_chunk_pages` 제거).
- **데이터 현실**: 이 교재는 청크가 섹션 단위라 대표 청크도 개념에 따라 넓음(일부 3–17쪽). → EvidenceBadge에서 **페이지 ≤5일 때만 노출**(정확할 때만 주장, 넓으면 book 배지 억제·ai_prereq 링크는 유지). 완화가 아니라 신뢰 강화.
- **검증**: 8절 중 5절 정확 배지(예: "교재 4쪽")·3절 억제, tsc 0. 봉투는 대표 청크 기준으로 서빙.
- **후속(정공 ②)**: faithfulness 게이트가 블록 단위 근거 대조를 하니 판정 부산물로 "지지 청크 id"를 받아 블록별 `source_chunk_ids`를 교체하면 추가 LLM 콜 0으로 진짜 블록 단위 귀속 완성 → "블록 단위 근거 추적" 서사. (generator 수정 + 재생성 필요라 별도 트랙)
...
### C8 — external_refs 검색원 보강
- **원인**: `_search_wiki`가 `limit=1`로 **1위만** 봄 → 1위가 동음이의·목록 문서면 즉시 미스→LLM 폴백(히트율 19%). 429는 1회 재시도뿐.
- **해결**: `limit=5`로 상위 후보를 받아 **스킵 신호(동음이의·"목록" 접미사) 없는 첫 후보 채택**, 429는 **지수 백오프 3회**. 동시성 3(위키 예절) 유지.
- **검증**: 실 ai_prereq 개념 51개로 위키 실호출 → **web 히트율 19%→47%(2.5배)**. 미스는 "추상화·계층적 구조" 등 위키 단일 문서 없는 추상·합성어(LLM 폴백 정상 담당). 기존 코스는 재수집 시 소급(이미 근거 있으면 스킵이라 자동 아님).

### C11 — 노이즈 개념 출제 필터 (ISSUE-016④)
- **접근(이슈 의도 준수)**: 정제는 보수적으로 두고(개념은 DB에 남김) **진단 대상 선정에서만** 거른다. 단일 지점 = `onboarding.start`의 `_part_representatives` 결과.
- **구현**: `_drop_noise_reps` — 대표 개념명+설명을 LLM 배치 1콜로 "학습 개념 vs 표지·삽화·목차·저작권 잡음" 판정, 확실한 잡음만 제외. 오판 방어 2겹: 애매하면 남기라 지시 + 전부 잡음/판정 실패 시 원본 유지(진단이 비지 않게). 저장 없는 런타임 필터(스키마 0).
- **검증**: 현 DB엔 노이즈 개념 없음(정제 v1 효과) → 실 개념 4 + 인위 노이즈 3(무지개·물방울·한빛미디어) 혼합 → **노이즈 3개 정확 제외·학습 개념 유지**, 전부 잡음 케이스 원본 유지 ✅.

### C단계 종합
- **C8·C9·C10·C11 전부 완료** = C단계 클로즈. A·B·C 모든 단계 완료.
- 남은 후속(선택): 노이즈 필터를 학습 섹션 대표까지 확대(지금은 진단만), 기존 코스 external_refs 재수집.

### 사용자 요청
- C단계 착수, C10(진단 미완료 코스 트리 400)부터.

### 추론 / 결정
- **재현 결과 = 서버 400은 이미 소멸**: 진단 미완료 코스(diag not_started, floor/ceiling NULL)로 트리/mastery/cursor 전부 **200**, 절 21개 중 20개 정상 잠금, 첫 절 블록 서빙 200. 원인 = 소민섭 진단 재설계가 floor/ceiling을 잠금 계산에서 제거(seed/service.py: "ceiling 위 locked 잠금을 하지 않는다") → 이슈 표엔 미착수로 남아 있었으나 실제론 해결됨.
- **남은 실질 갭 = 프론트 UX**: 정상 경로(책장 카드)는 미완료 코스를 /diagnosis로 보내지만, **지식 지도 노드 클릭·URL 직접 진입**으로 오면 LearningPage가 diag_status를 안 봐서 잠긴 트리만 덩그러니 → 혼란. 학습을 막지는 않되(직접 온 사용자 존중) 진단 유도 배너로 안내.

### 한 일 (프론트 전용)
- LearningPage: `currentCourse.diagStatus !== "completed"` → 본문 상단에 "아직 수준 진단을 하지 않은 코스예요 · [진단 시작하기]"(→ /diagnosis/:courseId) 배너. useCourses 재사용, 백엔드 0.

### 검증
- 진단 미완료 코스 API 3종 200 재확인, tsc 0. 이슈 표 #5 → 해결.

### C단계 남은 것
- C8(external_refs 히트율 19%) · C9(문서 병렬 ingest) · C11(노이즈 개념 필터). 추천 순: C9(데모 체감) → C8 → C11.
...
### 사용자 요청
- 이어서 할 작업 리스트 중 **A단계(오늘 작업 직접 후속)** 진행 승인.

### 한 일
1. **A1 페어 확인문제 재생성** — 태그(afterConcept)로 요청됐던 확인 문제가 게이트에서 전멸한 조각만 1회 보충(`_regen_pair_problem`, 상한 3콜). 첫 생성과 동일 게이트 전부 통과 시에만 편입, meta.pairRegen 표식. "빈칸 정답 유일이 어려우면 mcq로" 지시 포함(같은 실패 반복 회피).
2. **A2 요약 노트 실저장** — 튜터 패널 노트 탭 껍데기 → 실기능. `study_notes`(user×section PK, **mig 0021**, section FK CASCADE라 코스 삭제와 정합) + GET/PUT `/sections/:id/note`(upsert) + 프론트 로드/저장/상태표시(저장 중·저장됨·실패·마지막 저장 시각).
3. **A3+ 튜터 접지 강화** — 톤만 부드럽게 바꾸니 **범위 밖 질문(양자컴퓨터)에 자기 지식으로 답변**(가드레일 붕괴 실측, 평문 지시 무시). `{"inScope", "reply"}` **JSON 출력 강제**로 판정 단계를 구조에 박아 해결.
4. **A4 1블록 절 실태·수리** — 파서버그(세션23) 소급 피해 조사: 생성된 17개 절 중 thin 절 **1개뿐**(데이터통신 'WAN', 2블록·문제 0). 0블록 절 22개는 전부 pending 챕터(미생성 — 피해 아님). WAN은 수술적 재생성으로 교체(2→7블록, 페어+표 포함).

### 검증
- A1: 스크립트 mock(결함 cloze→폐기→재생성 mcq)로 `concept → mcq(1,R) → explainBack` 정확 편입 ✅ + 실 Solar 회귀(재생성 경로 활성) ✅
- A2: 실 API PUT→GET 왕복(내용·updatedAt 보존) ✅, tsc 0
- A3: 3종 시나리오 실 Solar — 범위 밖=설명 0+부드러운 안내 ✅ / 범위 안=자연 설명 ✅ / 정답 요구=비유출 ✅
- A4: WAN 절 7블록 교체 확인 ✅

### 다음 액션
- B5 조사(Document Parse figure 좌표) → Layer 2. B6 온디바이스 통합(결선 대비 큰 산). C 백로그(위키 히트율·병렬 ingest·진단 미완료 400·노이즈 필터).
...
### 브랜치 방침 (세션 후반 결정, 사용자 지시)
- **7/14 작업 전체(세션22~25, 6커밋, tip 099e209)는 `feat/yoonhs-work` 개인 브랜치에만 둔다.** `feat/yoonhs-integration`은 잠시 Layer 1(b8f90e1)까지 FF돼 있던 것을 **1b024af(세션21)로 force-push 되감음**(사용자 실행, --force-with-lease, 로컬=origin 확인). integration 머지는 사용자가 명시할 때만. 팀원이 낮 사이 b8f90e1 기준으로 pull했을 가능성 → 소민섭에게 되감기 공유 권장.
- 사용자 육안 검증 완료: 페어 구조·구조화 박스·비교표/다이어그램·튜터 FAB 실 화면 확인.
