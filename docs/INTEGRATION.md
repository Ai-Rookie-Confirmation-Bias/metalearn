...
GET    /api/courses                            내가 만든 수업 목록 (문제집 과목 선택 화면도 이 목록을 씀)
...
GET    /api/curriculum/analysis                           ★메타인지 분석 — 내 자료 전체 누적
```
채점 바디: `{correct, conceptKey, kind}` · `kind` ∈ `diagnostic|retrieval|review|formative`

**`/analysis`는 계산을 안 한다.** `summarize()`가 자료별로 낸 결과를 모아 **화면 수로
가중해** 합칠 뿐이다(단순 평균을 쓰면 3화면 샘플이 124화면 교재와 같은 무게가 된다).
응답의 `byKind`가 이 서비스의 주장을 그대로 드러내는 자리다 — **비어 있는 출처가 곧
빈 구멍**이라, 진단이 0이면 누적이 사실상 3출처로 돌고 있다는 뜻이다.
실측(08-08): `{retrieval: 4}` 하나. 화면은 이 0을 감추지 않고 경고로 띄운다.
...
### 프론트 — ✅ 진단 화면 (08-08, 박지성 설계 그대로)

`/curriculum/:courseId/diagnostic`. **코스에서만 열린다** — 파싱 자료 하나엔 진단이
없다. 진입은 학습 화면 헤더의 🔁 복습 옆.
...
- **④가 두 단계다.** 항목을 전부 물으면 54개다. 과목으로 먼저 묻고 "들어봤다"인
  것만 펼치면 실측 7 + 23 = 30번이 된다. 실측(08-08) 과목 10 · 항목 36.
...
- ③ 카드는 LLM 1콜이라 십수 초 걸린다. 데모에서 여기가 제일 오래 멈추는 자리다.
- ③ 카드 본문은 **마크다운을 그대로 렌더한다**(표 카드가 있다). 개행을 죽이면
  표가 원문으로 보인다 — 실제로 한 번 겪었다.

### 프론트 — ✅ 메타인지 분석 화면 (08-08)

`/analysis`. 22줄 placeholder였다. `GET /api/curriculum/analysis` 하나를 부른다.

- **제일 크게 보여주는 건 준비도 하나가 아니라 "그게 무엇으로 이루어졌는가"**다.
  출처별 막대 + 가중치(×0.5~×2.0) + 왜 그 무게인지.
- 가중치 표기는 `mastery.WEIGHT`와 **같아야 하는 값**이라 화면에 주석으로 묶어 뒀다.
  백엔드에서 무게를 바꾸면 여기도 고쳐야 한다.
