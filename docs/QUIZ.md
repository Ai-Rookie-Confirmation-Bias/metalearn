...
### ⑥ 검증 — `core/quality` 공통 모듈 (문제은행·학습 페이지가 공유)

검증은 quiz 기능이 아니라 **`app/core/quality/`의 공통 검증기**가 담당한다.
입력 계약은 "문항(type+data) + 근거 텍스트 + LLM"뿐 — 조각·목차·코스 개념이 없어
학습 페이지의 JIT 생성·진단·형성평가 문항도 같은 `validate_items()`를 호출한다.
(README 폴더 계획의 `core/verify_grade.py` 자리를 이 패키지가 맡는다.)

내부 단계 (연구·상용 사례 조사를 반영, docs/QUIZ_TUNING.md §8):

1. **오답 선지 선별(polish)**: mcq는 오답 후보를 과생성(`distractorPool`)받아
   중복·정답 동치 제거 후 길이 동질성 순으로 3개 선별 (overgenerate-and-rank).
2. **기계 검사(코드)**: 선지 정확히 4개, cloze 정답의 근거 존재·지문 노출, sN 유출 등.
3. **LLM 심판(묶음)**: 체크리스트 Q1~Q11 — 근거 대조 + 정답 유일성 질문.
4. **풀이 왕복(묶음)**: 정답을 가린 문항을 풀이자 LLM이 근거만 보고 실제로 푼다.
   키 정답 불일치·복수 정답 판단이면 탈락 — 유일성을 묻지 않고 **실험**한다.
   (정답 키 없는 유형(explainBack 등)은 이 단계 제외)
5. **불합격 수정 1회(critique-revise)**: 탈락 문항을 사유와 함께 되돌려 data만
   수정시키고 2~4를 재검사. 회생 실패면 최종 폐기.

- **기출 유사도 검사**: 실제 기출 문항과 과도하게 유사하면 폐기 (복제 방지 — 유료 기출집 저작권 방어선).
...
## 3.5 UX 결정 — 학습 기록의 사용 범위 (08-06 확정)

문제 페이지에 "최근 학습 과목 추천 배너"와 목차별 "📖 학습함" 라벨을 추가하면서
확정안 §7-③(두 페이지 데이터 격리)의 선을 다음과 같이 확정했다:

```
허용: 학습 기록 → "어디를 풀지" 안내      (추천 배너, 학습함 라벨 — 탐색 편의)
금지: 학습 기록 → "무엇이 나오는지" 결정   (문항 선정·구성·순서 — 은행은 모두에게 동일)
금지: 문제 풀이 결과 → 학습/커리큘럼 반영   (역방향 차단은 그대로)
```

- 격리 원칙의 알맹이(① 문제은행 비개인화 ② 풀이 결과의 커리큘럼 오염 차단)는 유지.
- 범위 선택 화면의 ⚡ 문구는 "학습 기록을 보지 않습니다" → **"문제는 학습 기록과
  무관하게 모두에게 같습니다. 학습 기록은 추천과 학습함 표시에만 쓰여요"**로 수정
  (거짓말이 되지 않게).

### 백엔드 TODO (나중에 — 프론트는 mock으로 이미 동작)

1. **자리 예약**: `GET /courses/:id/quiz` 응답에 `last_activity_at: null`,
   목차별 `studied: false` 필드 추가 (항상 기본값 반환 — API 계약 고정용, 10분)
2. **실제 집계**: 학습 쪽 테이블(`attempts`, `section_progress`)이 마이그레이션된 뒤 —
   `last_activity_at = MAX(학습 attempts.created_at)` (책장과 같은 계산값 재사용),
   `studied = section_progress를 목차 단위로 집계`. **지금 못 하는 이유:** 원본 테이블이
   설계(SCHEMA.md)만 있고 실제 DB에 없음 (학습 백엔드 스텁 상태).

## 3.6 기출 스타일 별도 모드 (08-06 결정 — UI 완성, 백엔드 대기)

§2-③은 원래 "기출이 있으면 은행 전체를 기출 스타일로 생성"이었으나,
**기출 스타일을 별도 모드로 분리**하기로 결정:

```
기출(kind=exam) 올린 과목  → [일반 문제 풀기] + [📝 기출 스타일로 풀기]  버튼 2개
교재만 올린 과목           → [문제 풀러 가기]                            버튼 1개
```

- **일반 모드**: 다양한 유형으로 개념 확인 (지금의 은행)
- **기출 스타일 모드**: 기출의 유형 비율·발문 말투 모방 + 빈출 개념 위주 = 실전 모의
  느낌. **기출 문항 복제는 여전히 금지** — 내용·정답·근거는 전부 교재 원문에서
- 사용자 인과가 명확해짐: "기출을 올리면 그 과목에 모의고사 모드가 생긴다"
- 화면의 ⚡ 문구(기출 모드): "올리신 기출의 문제 유형과 발문 방식을 학습해 만든
  문제입니다. 기출 문항을 그대로 내지 않으며, 내용과 근거는 전부 교재 원문에서
  나옵니다."

### 백엔드 TODO (선행: 기출 스타일 프로파일 추출 ③ ← 팀원 기출 파싱 형식 확정)

1. `quiz_items`에 `style` 컬럼 (`standard` | `exam`)
2. 생성 배치: 기출 프로파일이 있으면 **2차 패스**로 exam 세트 생성 — 기출 빈출 개념
   위주(개념당 1~2), stem_patterns·typeDistribution 적용. 비용 ~+40% (기출 과목만)
3. 요약 API에 `has_exam_style: bool`, 세션 API에 `style` 파라미터
4. 프론트는 mock으로 이미 동작 (`mock.ts`의 `QuizStyle`·`has_exam_style` — API 계약과 1:1)
