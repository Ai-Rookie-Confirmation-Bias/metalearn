...
- 빈칸 정답은 조사(을/를·이/가·은/는·의·에·로 등)와 어미를 뺀 **핵심 용어 그 자체**여야 한다 — "단어처럼 딱 끊어지는" 형태. 조사·어미는 빈칸 밖 문장에 남겨라.
  (X) 문장 "…허가받지 않은 {{blank}}을 막는다" + 정답 "접근을"
  (O) 문장 "…허가받지 않은 {{blank}}을 막는다" + 정답 "접근"
- 받침에 따라 형태가 갈리는 조사(을/를·이/가·은/는·과/와)가 빈칸 바로 뒤에 오면, 정답의 받침 유무를 알 수 없어 비문이 될 수 있다 → "{{blank}}을(를)"처럼 병기하거나, 빈칸을 명사 앞("{{blank}} 방식")에 두어 조사 문제를 피하라. 절대 조사를 빈칸 안으로 넣어 해결하지 마라.
...
1) 개념을 원문 근거로 **충분히 설명**하되, 한 덩어리가 아니라 **의미 단위
   ...
   **가독성: body는 한 덩어리로 쓰지 말고 2~3문장마다 빈 줄(\\n\\n)로 문단을
   ...
   concept에는 body 외에 선택 필드를 적극 활용하라 — **각각 텍스트가 아니라
   전용 그래픽으로 렌더되니, 산문으로 다 쓰지 말고 아래 필드로 쪼개 담아라**:
   - "whyItMatters": 이 개념이 왜 중요한지 1~2문장.
   - "steps": 동작 원리·절차·단계 진행이 근거에 있으면 **3~5개의 짧은 단계
     배열**(각 항목은 한 구절, 완결 문장 금지 — 예: ["요청 수신", "인증 확인",
     "응답 반환"]). 화면엔 번호가 매겨진 단계 흐름 그래픽으로 그려진다. 흐름이
     없으면 넣지 마라. body에서 절차를 산문으로 나열하지 말고 여기로 빼라.
   - "example": 근거 범위 안의 구체 예시 1개(body에 쓴 예시의 반복 금지).
   - "misconception"/"misconceptionReality": 흔히 오해·혼동하는 지점이 근거에서
     구분·대비된 경우에만(억지로 만들지 마라). **"misconception"엔 틀린 통념을
     한 구절로 짧게, "misconceptionReality"엔 실제로 맞는 내용을 한 구절로 짧게**
     — 둘이 대비되게. 화면엔 ❌ 오해 / ✅ 실제로는 대비 카드로 그려진다.
2) **concept 조각마다, 그 조각만 읽으면 풀 수 있는 가벼운 확인 문제(cloze
   ...
   붙여라(배치는 시스템이 한다).
3) **이 절은 텍스트 벽이 되면 안 된다 — 근거 안에서 시각화할 거리를 적극적으로
   찾아라.** analogy(비유)·table(비교표)·diagram(도식)을 관련된 concept 조각
   바로 옆에 배치한다. 대부분의 학습 내용에는 비교(→table)나 흐름·구조(→diagram)
   중 적어도 하나가 들어 있다. **가능하면 이 절에 table 또는 diagram을 최소 1개
   포함하라.** 단, 아래 조건에 실제로 맞는 경우에만 만들고 근거에 없는 관계·비교를
   지어내지는 마라(억지 시각화 금지 — 없으면 넣지 않는 게 맞다).
   - **나열·비교가 문단보다 명확한 내용(종류·유형·계층·단계별 특징·장단점 등)이
     근거에 있으면 table 블록**을 만들어라 — 열 2~4개, 행 2~6개, 셀은 짧은 구·단어로.
   - **절차·흐름·구조 관계(단계 진행, 계층 통과, 포함·의존·인과 관계)가 근거에
     있으면 diagram 블록**을 만들어라 — 노드 2~8개(id는 N1, N2… 형식, label은
     40자 이내), 화살표(edges)는 **근거에 명시된 관계만**. Mermaid 코드는 쓰지
     마라 — 노드와 화살표 JSON만.
...
{{"blocks": [
  {{"type": "concept", "difficulty": "mid", "data": {{"title": "...", "body": "...", "whyItMatters": "...", "steps": ["단계1", "단계2", "단계3"], "example": "...", "misconception": "틀린 통념(짧게)", "misconceptionReality": "실제로는(짧게)"}}}},
...
# ── 교재 그림 설명 ────────────────────────────────────────────────────────────
# 교재 그림(doc_figures)은 서버가 붙이는데, 설명이 없으면 "그림만 덩그러니" 놓인다.
# solar-pro3는 이미지 입력을 못 받으므로(비전 미지원, 실측 2026-07-22) 그림 픽셀을
# 직접 볼 순 없다 → 그림 '주변 원문 + 개념'을 근거로 그림이 무엇을 나타내는지 안내
# 문구를 생성한다. 다만 주변 텍스트가 그림과 무관할 수도 있으므로(한 지면에 여러
# 주제가 섞임) LLM이 '이 텍스트가 이 그림 얘기인지'를 먼저 판단하게 한다 — 인접하다고
# 무조건 그 그림 설명이라 단정하지 않는다. 환각 방지: 원문에 없는 세부는 지어내지 않는다.
async def explain_figure(
    ...
    *,
    ...
    """교재 그림에 붙일 짧은 안내 설명(1~2문장). 실패/무관/빈값이면 None(설명 없이 진행)."""
    if not excerpt.strip():
        return None
    prompt = (
        "너는 학습 콘텐츠 작가다. 교재 그림에 학습자용 안내 설명을 붙인다.\n"
        f"학습 개념: {concept_name}\n"
        f"개념 설명: {concept_description or '(없음)'}\n\n"
        f"[교재에서 이 그림 '바로 주변'에 있던 원문]\n{excerpt[:1500]}\n\n"
        "이 원문은 그림 근처에 있던 텍스트일 뿐, 반드시 이 그림을 설명하는 건 아니다.\n"
        "한 지면에 여러 주제가 섞여 있어 주변 텍스트가 딴 얘기일 수 있다.\n"
        "먼저 판단하라: 위 원문이 '이 그림이 무엇을 나타내는지' 실제로 뒷받침하는가?\n"
        "- 뒷받침하면(related=true): 학습자가 이 그림이 '무엇을 나타내는 자료인지' "
        "파악하도록 1~2문장으로 설명을 쓴다. **원문에 없는 수치·이름·시각적 세부는 "
        "지어내지 마라** — 원문이 뒷받침하는 핵심과 그림의 역할만 안내한다.\n"
        "- 그림과 무관하거나 무엇을 나타내는지 원문으로 알 수 없으면(related=false): "
        "explanation을 빈 문자열로 두라. 억지로 지어내지 마라.\n"
        '출력 JSON: {"related": true/false, "explanation": "..."}'
    )
    try:
        raw = await llm.generate(prompt, json_mode=True)
        payload = json.loads(raw[raw.find("{") : raw.rfind("}") + 1])
        if payload.get("related") is False:  # 그림과 무관 판정 → 설명 생략
            return None
        text = str(payload.get("explanation") or "").strip()
        return text or None
    except Exception:  # noqa: BLE001 — 설명 생성 실패는 그림만 표시(net-additive)
        logger.warning("그림 설명 생성 실패 — 설명 없이 그림만: concept=%s", concept_name)
        return None
