...
# 직접 볼 순 없다 → 그림이 실린 '지면 원문 + 개념'을 근거로 그림이 무엇을 나타내는지
# 안내 문구를 생성한다. 환각 방지: 원문에 없는 시각적 세부·수치는 지어내지 않는다.
async def explain_figure(
    ...
    *,
    ...
    """교재 그림에 붙일 짧은 안내 설명(1~2문장). 실패/빈값이면 None(설명 없이 진행)."""
    if not excerpt.strip():
        return None
    prompt = (
        "너는 학습 콘텐츠 작가다. 교재에서 가져온 그림에 학습자용 안내 설명을 붙인다.\n"
        f"학습 개념: {concept_name}\n"
        f"개념 설명: {concept_description or '(없음)'}\n\n"
        f"[이 그림이 실린 교재 지면 원문]\n{excerpt[:1500]}\n\n"
        "위 지면에 실린 교재 그림에 붙일 안내 설명을 작성하라. 학습자가 이 그림이 "
        "'무엇을 나타내는 자료인지' 파악하고 위 학습 개념과 연결하도록 1~2문장으로 "
        "쓴다. **원문 근거에 없는 구체적 수치·이름·시각적 세부는 지어내지 마라** — "
        "원문이 다루는 핵심과 그림의 역할만 안내한다.\n"
        '출력 JSON: {"explanation": "..."}'
    )
    try:
