...
기준의 문구와 표현이 달라도 의미가 충족되면 O다 — 교재 문구 그대로 쓸 필요 없고,
동의어·자기 말로 풀어쓴 설명·타당한 다른 예시도 인정한다.
...
_CLOZE_JUDGE_SYSTEM = (
    "너는 빈칸 채우기 채점관이다. 학습자 답을 이 빈칸의 답으로 인정할 수 있는지 "
    "판정한다. 출력은 JSON만."
)
...
async def grade_cloze_llm_blanks(
    ...
    판정 기준은 '출제 정답과 동의어인가'가 아니라 '이 빈칸의 답으로 인정 가능한가':
    정답이 유일하지 않은 문항(생성 게이트의 구조적 잔여 — WORK_LOG 2026-07-12(2))에서
    문맥상 똑같이 성립하는 대안 답을 낸 학습자에게 불이익을 주지 않기 위해서다.
    호출/파싱 실패 시 전부 False. 반환: blanks와 같은 길이의 bool 배열."""
    pairs = "\n".join(
        f'{i}. 정답="{blanks[i]}" / 학습자답="{parts[i] if i < len(parts) else ""}"'
        for i in range(len(blanks))
    )
    prompt = (
        "너는 빈칸 채우기 채점관이다. 각 빈칸에서 학습자 답을 채점한다.\n"
        "다음 중 하나면 정답(true):\n"
        "A) 출제 정답과 의미가 같다 — 표기 차이(공백·기호·괄호), 어순, 동의어, "
        "수식의 다른 표기(예: 'f''(a)=0'와 'f''(a) = 0', '√(ax²+ay²)'와 "
        "'sqrt(ax^2+ay^2)')는 전부 허용.\n"
        "B) 출제 정답과 다른 말이지만, 빈칸에 넣었을 때 문장이 사실적으로 옳고 "
        "자연스럽게 성립한다 — 정답이 유일하지 않은 문항에서 타당한 대안 답을 낸 "
        "학습자에게 불이익을 주지 않는다.\n"
        "오답(false): 핵심 의미·값이 다르면서 B에도 해당하지 않는 경우. "
        "'것'·'방법'처럼 어느 빈칸에나 들어가는 공허한 답은 지식 확인이 안 되므로 "
        "오답.\n"
        "B인지 애매한 경계 사례는 학습자에게 유리하게 정답으로 인정한다"
        "(문항 결함의 비용을 학습자가 지지 않는다).\n"
        f"문장: {text}\n"
        f"빈칸별 (정답 / 학습자답):\n{pairs}\n"
