...
_JUDGE_SYSTEM = (
    "너는 엄정한 채점관이다. 학습자 답을 이 문항의 정답으로 인정할 수 있는지 "
    "판정한다. 출력은 JSON만."
)
...
def _judge_prompt(question: str, expected: str, answer: str) -> str:
    # 기준 확장(2026-07-13): '모범답안과 의미 일치'만 보면 정답이 유일하지 않은
    # 문항에서 타당한 대안 답이 전부 오답 처리된다(BKT 오염). B 기준으로 흡수.
    return (
        f"문항: {question}\n"
        f"모범답안: {expected}\n"
        f"학습자 답: {answer}\n\n"
        "다음 중 하나면 correct=true:\n"
        "A) 모범답안과 의미가 본질적으로 일치 — 표현/어순/동의어/사소한 오타 차이는 "
        "허용.\n"
        "B) 모범답안과 다른 말이지만 문항에 대한 답으로 사실상 옳음 — 정답이 "
        "유일하지 않은 문항에서 타당한 대안 답을 낸 학습자에게 불이익을 주지 않는다.\n"
        "false: 핵심 개념이 틀리거나, 비어 있거나, 어느 문항에나 통하는 공허한 답"
        "(예: '것', '방법').\n"
