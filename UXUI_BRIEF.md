...
- **페이지 흐름:** 랜딩 → (가입/로그인) → 라이브러리 → 수업 생성 → 학습 → 마이페이지. **우선순위: 학습·수업 생성.** (가입 전 온보딩은 랜딩과 중복되어 제외)
...
- [x] 랜딩 `/` (+ `InteractiveDemo` 데모) · 헤더 로그인 · `/login` 스텁
- [~] ~~가입 전 온보딩~~ — 랜딩과 중복이라 **funnel에서 제외.** 그 위저드 껍데기는 `CreateCoursePage.tsx`(**수업 생성**)로 파킹(라우트 미연결, 내용은 교체 예정)
- [ ] 학습 화면 (핵심 — 데이터→렌더 설계 동반)
- [ ] 라이브러리 / 마이페이지(대시보드) / focus / **수업 생성**(create_course — 파킹된 위저드 이식)
