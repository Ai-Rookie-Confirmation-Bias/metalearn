...
export function DiagnosticBanner({ docId }: { docId: string }) {
  ...
  if (data.diagnosed_at) {
    return (
      <Link
        ...
        className="mb-4 inline-flex items-center gap-1.5 text-[0.8rem] text-text-tertiary hover:underline"
      >
        <ClipboardTextIcon className="text-[0.95rem]" />
        진단 리포트 보기
      </Link>
    );
  }
  ...
  return (
    ...
    >
      <CompassIcon
        className="shrink-0 text-[1.75rem] text-accent"
        weight="duotone"
      />
      <div className="min-w-0">
        ...
        <div className="mt-0.5 text-[0.88rem] text-text-secondary">
          시험이 아니에요. 아는 것은 설명을 줄이고 모르는 것은 먼저 채우려고
          묻습니다.
        </div>
      </div>
