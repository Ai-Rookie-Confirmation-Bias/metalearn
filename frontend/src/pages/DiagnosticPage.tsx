...
const STEPS = ["왜 배우나", "분야", "설명 방식", "아는 것", "확인"];
...
export function DiagnosticPage() {
  ...
  const [done, setDone] = useState<{ demoted: string[]; items: number } | null>(null);
  ...
  async function next() {
    ...
    try {
      if (step === 0) {
        ...
        const g = await gradeProbes(docId, results);
        setDone({ demoted: g.demoted_subjects, items: g.demoted_items });
      }
    } catch {
  ...
  async function toProbes() {
    ...
    setProbes(p);
    // ⑤는 **빈 목록일 수 있다** — 정답을 못 세운 항목은 문항을 안 낸다.
    if (p.length === 0) setDone({ demoted: [], items: 0 });
    else setStep(4);
  }
      ...
      <div className="mx-auto max-w-[680px] px-8 py-24 text-center">
        ...
        {done.items > 0 && (
          <p className="mt-4 rounded-xl bg-bg-secondary px-5 py-3 text-[0.9rem] text-text-secondary">
            확인 문항에서 틀린 게 있어 <strong>{done.demoted.join(" · ")}</strong>{" "}
            {done.demoted.length > 1 ? "과목들을" : "과목을"} 다시 보기로 표시했어요
            ({done.items}개 항목).
          </p>
        )}
        <button
    ...
    <div className="mx-auto w-full max-w-[760px] px-8 py-12">
      ...
      {step === 4 && (
        <>
          ...
          <p className="mb-7 text-[0.95rem] text-text-secondary">
            맞히면 그대로 넘어가고, 틀리면 그 과목을 다시 보기로 표시해요.
          </p>
          <div className="flex flex-col gap-6">
            {(probes ?? []).map((p, i) => (
