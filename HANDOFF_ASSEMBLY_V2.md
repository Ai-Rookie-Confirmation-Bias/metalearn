...
5. **[🟡 품질/UX] 잘못된 문항 + 오답 피드백 부재 (ISSUE-016)** — 커밋 `a309a3f`
   ...
   - 수정(`diagnostic/service.py`·`schemas.py` + 프론트 2곳) — 문항 품질은 **3중 방어**:
     A-1. **생성 프롬프트**(`_quiz_prompt`·`_batch_quiz_prompt`): inverse는 "설명→개념
        이름 맞히기"이니 개념이 사람 아니면 문두를 '누구'로 시작 금지. 실측: '도덕적
        주체' 생성 6/6 정상(누구 0).
     A-2. **결정적 가드**(`_verify_drafts` 선처리): inverse 문두에 '누구/누가'인데
        정답에 인물 마커 없으면 무조건 불합격→재생성. LLM 유형검증이 'agent'류
        개념에 2/5만 잡던 것을 **5/5로** 보완. 인물 개념(정답에 사람/학자 등)은 통과.
     A-3. `_verify_prompt`에 문두↔정답 유형 하드 게이트(LLM 백업, 확률적).
     B. `_grade_with_feedback` 신설로 심판 rationale 표면화(추가 비용 0, mcq는
        ...
        "왜 틀렸나:" 표시.
   - ⚠️ 개선은 **새로 생성되는** 문항부터 적용 — DB의 기존 문항은 재진단/재생성 시 반영.
   - ⚠️ 교훈: LLM 자기검증만으론 borderline(agent류) 케이스가 확률적으로 샌다 —
     생성 억제 + 결정적 룰과 병행해야 신뢰 가능.
